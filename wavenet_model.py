...
class WaveNetModel(nn.Module):
    ...
    def wavenet(self, input, dilation_func):
        ...
        print("input inside wavenet model = ",x.size())
        skip = 0
        print("no of iterations: ", self.blocks * self.layers)
        # WaveNet layers
        for i in range(self.blocks * self.layers):
            ...
            print("x before residual: ", x.size())
            ...
            if x.size(2) != 1:
                 ...
                 print("after dilation: ", s.size())
            s = self.skip_convs[i](s)
            print("s==== ",s.size())
            print("s.size(2) ",s.size(2))
            try:
                skip = skip[:, :, -s.size(2):]
            except:
                skip = 0
    ...
    def forward(self, input):
        print("input in forward",input.size())
